...
# Part A: Abstract Employee Class
...
# 4. Regular method pay_stub() that returns: "[name] (ID: [employee_id]): $[pay]" (calls calculate_pay() internally — this is polymorphism!)
...
# Part B: Concrete Employee Classes
# Create three concrete classes that inherit from Employee:
...
#calculate_pay() returns hourly_rate * hours_worked for the first 40 hours, plus hourly_rate * 1.5 * overtime_hours for any hours beyond 40
...
class CommissionEmployee(Employee):
    def __init__(self, name, employee_id, base_salary, sales, commission_rate):
        ...
        if commission_rate > 1.0:
            raise ValueError("Commission rate must be ≤ 1.0!")
